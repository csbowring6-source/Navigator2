...
`openSession()` · `requestSession()` (gated self-open — never overrides a shut-up) · `closeSession(reason)` · `toggleCapture()` · `micTap()` · `reset()` · `speak(text)` · `resumeSpeech(text)` (continue-words re-speak a freshly-cut answer) · `queueReplies(on)` · `cancelSpeech()` · `unlockAudio()` · `state()` · `isSessionOpen()` · `isCapturing()` · `canHandsFree()` · `onTranscript(cb)` · `setBusyGetter(fn)` · `takeTurnEnd()` · `log(kind,detail)` · `getLog()` · `clearLog()` · `BUILD`.
