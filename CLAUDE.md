...
`openSession()` · `requestSession()` (gated self-open — never overrides a shut-up) · `closeSession(reason)` · `toggleCapture()` · `micTap()` · `reset()` · `speak(text)` · `queueReplies(on)` · `cancelSpeech()` · `unlockAudio()` · `state()` · `isSessionOpen()` · `isCapturing()` · `canHandsFree()` · `onTranscript(cb)` · `setBusyGetter(fn)` · `takeTurnEnd()` · `log(kind,detail)` · `getLog()` · `clearLog()` · `BUILD`.
